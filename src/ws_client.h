 ...
 *
 * 流控
 *      - wsld_set_queue_limit() 设置最大消息数 | 字数上限, 任一触发返回 WS_ERR_QUEUE_FULL
 *      - 队列满时 wsld_send() 立即发送,不阻塞调用方
 *
 * 嵌入方式
...
typedef void (*wsld_rx_cb_t)(const char *data, size_t len, int is_binary, void *user) ;

typedef struct wsld_client wsld_client_t;

/*
 * wsld_create - 创建客户端实例(不发起连接)
 *
 * host:        服务端地址
 * port:        服务端端口
...
/*
 * wsld_set_heartbeat - 设置心跳相关参数(须在wsl_start 前调用)
 * 
...
/*
 * wsld_set_reconnect - 设置重连参数（须在 wsld_start 前调用）
 *
...
/*
 * wsld_set_queue_limit - 设置发送队列上限 (须在 wsld_start 前调用)
 *
 * max_msgs: 最大消息条数， 0 表示不限制
 * max_bytes: 最大总字节数， 0 表示不限制
 */
void wsld_set_queue_limit(wsld_client_t *c, int max_ms, int max_bytes);

/*
 * wsld_set_ssl - 启用/禁用 SSL/TLS (须在 wsld_start 前调用)
 *
...
/*
 * wsld_set_path - 设置 WebSocket 连接路径 (须在 wsld_start 前调用)
 *
...
/*
 * wsld_start - 启动 service 线程并发起首次连接
 *
 * 返回： 1 表示成功， 0 表示失败
 */
...
/*
 * wsld_stop - 优雅停止并等待 service 线程退出，同时销毁 lws_context
 * 之后可安全调用 wsld_destroy() 释放内存
 */
void wsld_stop(wsld_client_t *c);

/*
 * wsld_destroy - 释放 wsld_client_t 及其所有内存
 * 必须在 wsld_stop() 之后调用，否则 service 线程仍在运行，行为未定义
 */
void wsld_destroy(wsld_client_t *c);

/*
 * wsld_send / wsld_send_binary -线程安全发送
 * data 拷贝后立返回，不阻塞调用方
 * 连接断开期间消息仍会入队，连接后发送
 */
